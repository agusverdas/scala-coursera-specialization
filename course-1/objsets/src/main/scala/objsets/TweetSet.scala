...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def max(x: Tweet, y: Tweet) = x.retweets - y.retweets match {
    case value if value < 0 => y
    case _ => x
  }
  ...
  def descendingByRetweet: TweetList = {
    ...
    val tailTweetSet = this remove mostRetweeted
    new Cons(mostRetweeted, tailTweetSet.descendingByRetweet)
  }
}
