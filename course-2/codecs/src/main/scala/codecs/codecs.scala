...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromPartialFunction { case Json.Arr(items) => items.map(decoder.decode(_).get) }
...
trait ContactsCodecs {
  ...
  implicit val contactsEncoder: Encoder[Contacts] = Encoder.fromFunction(p =>
    Json.Obj(Map("people" -> Encoder.listEncoder[Person].encode(p.people))))

  implicit val contactsDecoder: Decoder[Contacts] = Decoder.fromPartialFunction {
